 ...
 */
std::vector<long long> IndexManager::getLatestTimestamps(const std::string& component, size_t count) const {
    auto timestamps = getTimestamps(component);
    if (timestamps.empty()) return {};

    auto start = timestamps.size() > count ? 
                 timestamps.end() - count : 
                 timestamps.begin();
 ...
 */
std::vector<long long> IndexManager::getOldestTimestamps(const std::string& component, size_t count) const {
    auto timestamps = getTimestamps(component);
    if (timestamps.empty()) return {};

    auto end = timestamps.size() > count ? 
               timestamps.begin() + count : 
               timestamps.end();
 ...
 */
void IndexManager::deleteTimestamps(const std::string& component, const std::vector<long long>& timestampsToDelete) {
    auto it = index.find(component);
    if (it != index.end()) {
        ...
        timestamps.erase(
            std::remove_if(timestamps.begin(), timestamps.end(),
                [&](long long ts) {
                    return std::find(timestampsToDelete.begin(), timestampsToDelete.end(), ts) != timestampsToDelete.end();
                }
            ),
            timestamps.end()
        );
